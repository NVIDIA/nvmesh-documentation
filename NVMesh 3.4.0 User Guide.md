...
| **CDV Extent Size** | The granularity at which CDV space is divided and leased to TPVs. Must be a power of 2 in the range 16 MiB–64 GiB. Smaller values provide finer-grained allocation at the cost of higher metadata overhead on the CDV's satellite volume. Total CDV capacity is bounded by the satellite size (see **Allocator Size**): approximately `allocatorSizeGib × 262 143 × extent size` bytes; at the default 1 GiB satellite that works out to ~4 TiB at 16 MiB extent, ~16 TiB at 64 MiB, ~256 TiB at 1 GiB. For metadata-role CDVs backing split-mode TPVs, 16 MiB or 32 MiB is a reasonable choice; for data-role CDVs, 1 GiB is a common default. This parameter cannot be changed after creation. |
| **Allocator Size** | Size (in GiB) of the automatically-created `<CDV>-mgmt` satellite volume that holds the TOMA allocator's per-extent records. Defaults to 1 GiB. Each extent consumes one 4 KiB record, so satellite size × 262 143 sets the upper bound on the number of extents the CDV can hold (and thus on total CDV capacity for a given CDV Extent Size). Raise this at create time for very large CDVs at small extent sizes. This parameter cannot be changed after creation. |
