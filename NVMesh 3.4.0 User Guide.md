...
| **CDV Extent Size** | The granularity at which CDV space is divided and leased to TPVs. Must be a power of 2 in the range 16 MiB–64 GiB. Smaller values provide finer-grained allocation at the cost of higher metadata overhead on the CDV's satellite volume; larger values scale total CDV capacity (the 1 GiB satellite caps the CDV at roughly 262 143 extents). For metadata-role CDVs backing split-mode TPVs, 16 MiB or 32 MiB is a reasonable choice; for data-role CDVs, 1 GiB is a common default. This parameter cannot be changed after creation. |
